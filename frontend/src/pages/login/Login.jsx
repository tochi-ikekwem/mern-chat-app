		...
		<div className='flex flex-col items-center justify-center min-w-96 mx-auto'>
			<div className='w-full p-6 rounded-lg shadow-md bg-gray-500 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-0'>
				<h1 className='text-3xl font-semibold text-center text-gray-100'>
					Login
					<span className='text-gray-100'> ChatApp</span>
				</h1>
				...
				<form onSubmit={handleSubmit}>
					...
					<Link to='/signup' className='text-sm  hover:underline text-gray-100 hover:text-blue-600 mt-2 inline-block'>
						{"Don't"} have an account?
					</Link>

					<div>
						<button className='btn btn-block btn-sm mt-2 text-black' disabled={loading}>
							{loading ? <span className='loading loading-spinner text-gray-100 '></span> : "Login"}
						</button>
					</div>
				</form>
			</div>
